...
def gripper(state=False):
  ...
  while pub.get_num_connections() < 1:
    # print('gripper connect num ', pub.get_num_connections())
...
class MoveGroupPythonIntefaceTutorial(object):
  ...
  def move_test_pick(self,point_x,point_y,point_z,goalpoint_x,goalpoint_y,goalpoint_z,):
    ...
    try:
        ...
        pose_goal.position.z = point_z + 0.215
        move_group.set_pose_target(pose_goal)
        plan = move_group.go(wait=True)
  ...
  def move_vacuum_pick(self):
    ...
    try:
        ...
        self.home_pick_pose()
        # return
        # pre-pick 0.32517; 0.27695; 0.011454 + 0.05
        pose_goal = self.move_group.get_current_pose().pose
        pose_goal.position.x = 0.32517
        pose_goal.position.y = 0.26
        pose_goal.position.z = 0.011454 + 0.05
        move_group.set_pose_target(pose_goal)
        plan = move_group.go(wait=True)
        ...
        # post-place
        pose_goal = self.move_group.get_current_pose().pose
        pose_goal.position.x = 0.46038
        pose_goal.position.y = 0.11
        pose_goal.position.z = 0.56 - 0.325
        move_group.set_pose_target(pose_goal)
        plan = move_group.go(wait=True)
...
def main():
  ...
  while not rospy.is_shutdown():
    ...
    #   print('You Pressed A Key to quit')
